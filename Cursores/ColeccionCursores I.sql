...
NUM_EMP EMP.EMPNO%TYPE, -- TRATADO
NOMBRE EMP.ENAME%TYPE, -- TRATADO
TRABAJO EMP.JOB%TYPE, -- TRATADO
JEFE EMP.MGR%TYPE, -- TRATADO
FECHA EMP.HIREDATE%TYPE,
SALARIO EMP.SAL%TYPE, -- TRATADO
COMISION EMP.COMM%TYPE, -- TRATADO
NUM_DEPT EMP.DEPTNO%TYPE -- TRATADO
) IS
...
BEGIN
    ...
    ELSE
        IF NUM_EMP IN (SELECT EMPNO FROM EMP) OR NUM_EMP < 0 THEN
            RAISE EXCEP_NUM_INVALIDO;
        END IF;
    END IF;
    -- TRATAR MGR
    IF JEFE IS NOT NULL THEN
        IF JEFE NOT IN (SELECT EMPNO FROM EMP) THEN
            RAISE EXCEP_NUM_INVALIDO;
        END IF;
    END IF;
    -- TRATAR SAL
